    ...
    public sealed class CreateRecordAPI : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                if (entity.LogicalName != "stu7_inquiry")
                    return;
                ...
                try
                {
                    ...
                    using (WebClient client = new WebClient())
                    {
                        ...
                        var response = client.UploadString(new Uri("http://rest.learncode.academy/api/student7/inquiries/"), "POST", data);
                        var apiID = response.Substring(response.IndexOf("id\":\"") + 5, 24);
                        var currentRecord = _service.Retrieve("stu7_inquiry", entity.Id, new Xrm.Sdk.Query.ColumnSet(true));
                        currentRecord["stu7_name"] = apiID;// since we didn't have time to create a new apiId in CRM, we just reuse the 'stu7_name' field
                ...
                try
                {
                    ...
                    using (WebClient client = new WebClient())
                    {
                       // var apiId = postEntity.GetAttributeValue<string>("new_apiid");
                        var apiId = postEntity.GetAttributeValue<string>("stu7_name"); // since we didn't create an apiid filed, we use the stu7_name field for now
                        ...
                        client.Headers.Add(HttpRequestHeader.ContentType, "application/json");
                        var response = client.UploadString(new Uri("http://rest.learncode.academy/api/student7/inquiries/" + apiId), "PUT", data);
